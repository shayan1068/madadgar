...
import {getDeailsOfDonationPost,getRequestForDonation} from "./../donationhandler/donationHandler";
import swal from "sweetalert";
import FirstPic from "./../../../images/Donation/donationPicture.svg";
import * as Yup from "yup";
function NeedDonation() {

    const userId = sessionStorage.getItem('user');
    const [toggle, setToggle] = useState({ index: -1, toggle: false });
    const [open, setOpen] = useState({ doId: "", userId: "", status: false });
    ...
    const getfetchDataFromCollection = async () => {
        await getDeailsOfDonationPost(userId!).then((response: any) => {
            setData(response);
        })
    }
    useEffect(() => {
                        ...
                        <Card shadow="sm" className="toggle-card-bg">
                            <Grid>
                                <Grid.Col lg={12} md={12} sm={12} className="requirements">
                                    <Text className="sub-heading">Donation Stuff Details</Text>
                                </Grid.Col>
                            </Grid>
                            <Container fluid>
                                <Grid className="foj-conatiner">
                                    ...
                                    <Grid.Col lg={6} md={6} sm={6}>
                                        <Text> {getInfo.itemType}</Text>
                                    </Grid.Col>
                                    <Grid.Col lg={6} md={6} sm={6}>
                                        <Text> Size:</Text>
                                    </Grid.Col>
                                    <Grid.Col lg={6} md={6} sm={6}>
                                        <Text>{getInfo.size}</Text>
                                    </Grid.Col>
                                    <Grid.Col lg={6} md={6} sm={6}>
                                        <Text> Gender:</Text>
                                    </Grid.Col>
                                ...
                                <Grid className="foj-conatiner">
                                    <Grid.Col lg={12} md={12} sm={12}>
                                        <Group className="foj-footer">
                                            ...
                                            <Button className="foj-footer-btn" onClick={() => setOpen({ doId: getInfo._id.toString(), userId: getInfo.userId._id, status: true })}>Help In Education</Button>
                                        </Group>
                                    </Grid.Col>
                                </Grid>
                            </Container>
    ...
    return (
        <Container fluid className='root'>
                ...
                <Group>
                    <Grid className="job-row-text">
                        <Grid.Col lg={6} md={6} sm={12}>
                            ...
                            <Text className="job-banner-text">Help people who can not afford to complete their needs.
                            </Text>
                        </Grid.Col>
                        <Grid.Col lg={6} md={6} sm={12}>
                            <Image src={FirstPic} alt="banner image of jobs" className="img img-fluid w-75" />
                        </Grid.Col>
                    </Grid>
                </Group>
            </Grid>
            ...
            <Modal size="55%" radius="xl"
                ...
                opened={open.status}
                onClose={() => setOpen({ doId: "", userId: "", status: false })}
            >
                <Grid>
                    <Formik
                        initialValues={{ eid: open.doId, userIdAdmin: open.userId, newUserId: userId, name: '', gender: '', Phone: '', email: '' }}
                        validationSchema={EducationRequestscheme}
                        onSubmit={async (values) => {
                            try{
                                await getRequestForDonation(values.userIdAdmin, values.eid, values.newUserId!, values.name, values.gender, values.Phone, values.email).then((response: any) => {
                                    swal(response, {
                                        icon: "success",
                                        buttons: {},
                                        timer: 3000,
                                    }).then(()=> {setUpdateCount(updateCount + 1);setOpen({ doId: "", userId: "", status: false })})
                                })
                            }catch (err:any) {
                                swal(err, {
                                  icon: "error",
